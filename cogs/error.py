    ...
    async def on_command_error(self, ctx, e):
        ...
        if isinstance(e, commands.CommandNotFound):
            ...
            await self.bot.get_channel(814830540133498920).send(
                ...
                embed=discord.Embed(title="エラー詳細", description=f"```{error_msg}```")
            )
            return await ctx.send(embed=discord.Embed(title="予期せぬエラーが発生しました", description=error_msg + "開発者に報告してください。"))
