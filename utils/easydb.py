...
class EasyDB():
    ...
    def create_table(self, table_name: str, **values) -> None:
        ...
        **values:(キーワード引数)中身 {value:type}"""
        ...
        self.cur.execute(
            f"CREATE TABLE IF NOT EXISTS {table_name}({', '.join(m)})"
        )
        if not getattr(self, table_name, False):
            setattr(
                self,
                table_name,
                Table(table_name, self.conn, self.cur)
            )
...
class Table():
    ...
    def add_column(
        ...
        *
        ...
        self.cur.execute(
            f"ALTER TABLE {self.name} ADD {name} {typ}{first}{after}"
        )
